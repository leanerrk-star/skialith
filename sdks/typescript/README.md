...
Durable execution layer for AI agents — a TypeScript/Node.js SDK that wraps the Durable Agent Core HTTP sidecar.

## Installation

```bash
npm install @durable/agent-core
```
...
```typescript
import { DurableAgent } from '@durable/agent-core';

const agent = new DurableAgent({ agentId: 'research-agent-42' });
...
### `new DurableAgent(options)`

| Option | Type | Default | Description |
|---|---|---|---|
| `agentId` | `string` | required | Unique identifier for this agent instance |
| `sidecarUrl` | `string` | `http://localhost:8080` | URL of the Durable Agent Core sidecar |
