"""LangGraph checkpointer backed by durable-agent-core."""
...
def _require_langgraph():
    if not _LANGGRAPH_AVAILABLE:
        raise ImportError(
            "langgraph is required: pip install 'durable-agent-core[langchain]'"
        )


class DurableCheckpointer:
    """Drop-in LangGraph checkpointer that persists state via durable-agent-core.
